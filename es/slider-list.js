...
var getTransition = function getTransition(count, initialValue, currentSlide, cellAlign, wrapAround) {
  if (cellAlign === _types.Alignment.Left) {
    ...
    if (wrapAround) {
      var _slideTransition2 = 100 / (3 * count);
      var _currentTransition = initialValue - _slideTransition2 * (currentSlide - 1);
      return _currentTransition - _slideTransition2;
    }
    var _slideTransition3 = 100 / count * currentSlide;
    return initialValue - _slideTransition3;
  } else if (cellAlign === _types.Alignment.Right) {
    if (wrapAround) {
      var _slideTransition4 = 100 / (3 * count);
      var _currentTransition2 = initialValue - _slideTransition4 * (currentSlide - 1);
      return _currentTransition2 - _slideTransition4;
    }
    var _slideTransition5 = 100 / count * currentSlide;
    return initialValue - _slideTransition5;
  }
  return initialValue;
};
var getPositioning = function getPositioning(cellAlign, slidesToShow, count, currentSlide, wrapAround, move) {
  ...
  var horizontalMove = (0, _utils.rtlMultiplier)() * getTransition(count, initialValue, currentSlide, cellAlign, wrapAround);
...
var getSliderListStyles = exports.getSliderListStyles = function getSliderListStyles(children, currentSlide, animation, slidesToShow, cellAlign, wrapAround, speed, move, slideAnimation) {
